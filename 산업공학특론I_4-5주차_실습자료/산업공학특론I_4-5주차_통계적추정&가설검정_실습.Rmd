...
# 로그우도함수 정의
mle = function(param){
  -sum(dexp(x,rate = param, log = TRUE)) # pdf의 density
}
# 최적화 수행하여 모수 추정
# ? optimize

mle_opt = optimize(mle,c(0,10))
mle_opt # minimum -> 추정량 : Lambda
# objective : Likelihood Function Value
#optim()
```
...
# 예측값 기반 최소제곱함수 정의
lse <- function(param){
  pred <- param*exp(-param*x)  # 지수분포 Term
  sum((x-pred)^2)
}
lse_opt <- optimize(lse,c(0,10))
lse_opt # Minimum을 이용했을 때의 Likelihood 값
...
hist(x,col = 'grey',breaks = 50 , main = 'Density',probability = 1)
curve(dexp(x,rate = mle_opt$minimum),col = 
        'red' , add = T)
curve(dexp(x,rate = lse_opt$minimum),col = 
        'blue' , add = T)
